...
const AllClasses = ({ data, refetch, title }) => {
    ...
    const handleClass = () => {
        if (myclass === "All classes") {
            ...
            setClassData(data.filter(i => { return i.department === myclass }))
        }
    }
    useEffect(() => {
        setClassData(data)
        ...
        <>
            {data.length > 0 && <div>
                ...
                <div className='flex justify-center items-center space-x-4 my-10'>
                    <select value={myclass} onChange={(e) => { setMyClass(e.target.value) }} className='p-2 text-xl rounded-lg'>
                ...
                {
                    classData.length === 0 &&
                    <div className='my-10 mx-2 text-red-700 text-center'>*This Department have no Classes*</div>
                }
            </div>}
        </>
    )
